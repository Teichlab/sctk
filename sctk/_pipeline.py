...
def calculate_qc(adata, flags={'mito': r'^MT-', 'ribo': r'^RP[LS]', 'hb': r'^HB'}, extra_flags={}, extra_qc_vars=[], flag_only=False):
    if extra_flags:
    ...
    for metric in qc_vars:
        ...
        adata.obs[f'n_counts_{metric}'] = qc_tbls[0][f'pct_counts_{metric}'].values * qc_tbls[0]['total_counts'].values / 100
...
def filter_qc_outlier(
        ...
        min_pass_rate = 0.6, onesided=False, force=False
):
    ...
    if 'scrublet_score_z' in metrics:
        x_low, x_high, _ = fit_gaussian(adata.obs['scrublet_score_z'].values, n=10, xmin=-99, xmax=10, threshold=0.05)
        max_sz = x_high
        min_sz = x_low
...
def simple_default_pipeline(
    ...
    if not sc.__version__.startswith('1.6'):
        use_gpu = False
    ...
    if batch:
        if batch_method == 'bbknn':
            ...
            umap(adata, method='rapids' if use_gpu == 'all' else None, use_graph=f'neighbors_{key_added}', key_added=key_added, random_state=random_state, **umap_kw)
            if do_clustering:
                leiden(adata, flavor='rapids' if use_gpu else None, use_graph=f'neighbors_{key_added}', resolution=clustering_resolution, key_added=key_added)
        else:
            key_added = hm_kw.pop('key_added', 'hm')
            run_harmony(adata, batch=batch, key_added=key_added, **hm_kw)
            neighbors(adata, method='rapids' if use_gpu else 'umap', use_rep=f'X_pca_{key_added}', key_added=key_added, n_pcs=n_pcs, n_neighbors=n_neighbors)
            umap(adata, method='rapids' if use_gpu == 'all' else None, use_graph=f'neighbors_{key_added}', key_added=key_added, random_state=random_state, **umap_kw)
            if do_clustering:
                leiden(adata, flavor='rapids' if use_gpu else None, use_graph=f'neighbors_{key_added}', resolution=clustering_resolution, key_added=key_added)
    else:
        sc.pp.neighbors(adata, use_rep='X_pca', n_pcs=n_pcs, n_neighbors=n_neighbors)
        umap(adata, method='rapids' if use_gpu == 'all' else None, random_state=random_state, **umap_kw)
        if do_clustering:
...
def auto_zoom_in(
    ...
    for grp in groups:
        ...
        print([grp, i], end=' ')
        while True:
            leiden(ad, use_graph=use_graph, restrict_to=(prev_grp_key, [grp]), resolution=res, key_added=f'split{i}', **leiden_kw)
        ...
        if subclustered:
            ...
            new_groups = ad.obs[new_grp_key].cat.categories[~ad.obs[new_grp_key].cat.categories.isin(ad.obs[prev_grp_key].cat.categories)]
...
def integrate(ads, ad_prefices=None, ad_types=None, annotations=None, batches=None, join='inner', n_hvg=4000, pool_only=False):
    n_ad = len(ads)
    ...
    for i, ad in enumerate(ads):
        ...
        if ad_type == 'raw_norm':
            ...
            if not sp.issparse(ad.X):
                norm_ad = anndata.AnnData(X=sp.csr_matrix(ad.X), obs=ad.obs.copy(), var=ad.var.copy())
            else:
    ...
    pooled = anndata.AnnData.concatenate(*norm_ads, batch_key='dataset', batch_categories=ad_prefices, join=join)
...
def crossmap(adata, dataset='dataset', annotation='annot'):
    ...
    df = pd.DataFrame(
        np.dot(
            cross_table(adata, f'{ds1}_annot', annotation, normalise='yx').values/100.0,
            cross_table(adata, annotation, f'{ds2}_annot', normalise='xy').values/100.0
        ),
